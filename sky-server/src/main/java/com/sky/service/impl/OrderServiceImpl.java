...
public class OrderServiceImpl implements OrderService {
    ...
    public OrderSubmitVO submitOrder(OrdersSubmitDTO ordersSubmitDTO) {
        ...
        // 3.向订单明细表插入n条数据(订单明细表中的数据实际是由购物车中的数据提供的)
...
//    }
    ...
    public PageResult conditionSearch(OrdersPageQueryDTO ordersPageQueryDTO) {
        ...
        // 部分订单状态需要额外返回订单菜品信息，将Orders转换成OrderVO
    ...
    private String getOrderDishes(Orders orders){
        // 查询订单菜品详情信息(订单中的菜品及数量)
        List<OrderDetail> orderDetailList = orderDetailMapper.getByOrderId(orders.getId());

        // 将每一条订单菜品信息拼接为字符串(格式如：宫保鸡丁*3;)
    ...
    public OrderStatisticsVO statistics() {
        // 分别查询和统计待接单、待派送、派送中的订单数量
        Integer toBeConfirmed = orderMapper.countStatus(Orders.TO_BE_CONFIRMED);
        Integer confirmed = orderMapper.countStatus(Orders.CONFIRMED);
        Integer deliveryInProgress = orderMapper.countStatus(Orders.DELIVERY_IN_PROGRESS);


        // 将查询到的数据封装入OrderStatisticsVO中
